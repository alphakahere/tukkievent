      ...
      <header className="bg-white border-b border-gray-100 px-4 pt-12 pb-4 sticky top-0 z-40 md:hidden">
        <div className="max-w-lg mx-auto">
          ...
          <div className="flex gap-2">
            {[
              { id: "purchases" as const, label: `Achats (${sortedOrders.length})` },
              { id: "refunds" as const, label: "Remboursements (0)" },
            ].map((tab) => (
              <button
                key={tab.id}
                type="button"
      ...
      <div className="max-w-lg md:max-w-5xl mx-auto px-4 py-6">
        <div className="md:flex md:gap-8">
          ...
          <div className="flex-1">
            <h1 className="hidden md:block text-2xl font-bold text-gray-900 mb-4">Historique</h1>
            ...
            {activeTab === "purchases" ? (
              sortedOrders.length === 0 ? (
                <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="text-center py-20">
                  ...
                  <p className="text-base font-semibold text-gray-900 mb-1">Aucun achat</p>
                  <p className="text-sm text-gray-500 mb-6">Vos achats de billets apparaîtront ici</p>
                ...
                <div className="space-y-3">
                  {sortedOrders.map((order, index) => {
                    ...
                    return (
                      <motion.div key={order.orderId} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.05 }}>
                        <Link href={`/history/${order.orderId}`} className="block bg-white rounded-2xl p-4 border border-gray-100 hover:border-gray-200 transition-colors">
                          <div className="flex items-start justify-between mb-3">
                            <div className="flex-1 min-w-0">
                              <p className="text-sm font-semibold text-gray-900 line-clamp-1 mb-0.5">{order.event.title}</p>
                              <p className="text-xs text-gray-500">Acheté le {purchaseDate}</p>
                            </div>
                            <span className="ml-3 px-2.5 py-1 bg-emerald-50 text-emerald-700 text-xs font-semibold rounded-full shrink-0">Confirmé</span>
                          </div>
                          <div className="flex items-center justify-between">
                            ...
                            <div className="flex items-center gap-1.5">
                              <span className="text-sm font-bold text-primary">{order.total.toLocaleString()} FCFA</span>
              ...
              <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="text-center py-20">
                ...
                <p className="text-base font-semibold text-gray-900 mb-1">Aucun remboursement</p>
                <p className="text-sm text-gray-500">Vos remboursements apparaîtront ici</p>
              </motion.div>
            )}
          </div>
        </div>
